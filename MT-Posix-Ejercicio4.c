// Este programa crea n hilos usando memoria dinamica. Todos estos hilos se ejecutan al mismo tiempo. Cada uno recibe un parametro tipo estructura
...
void *routine(void* aStruct){//FLujo de instrucciones de un hilo. FUncion se convierte en un hilo
    ...
    printf("Hola mundo! Soy el hilo %ld. Recibí una estructura con los siguientes datos\n",pthread_self());
    printf("\nEntero: %d\n",thStruct->integer);
    printf("Flotante: %.3f\n",thStruct->flotant);
    printf("Cadena: %s\n",thStruct->str);
}
